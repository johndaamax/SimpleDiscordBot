const ytList = {
    link: [
        'https://www.youtube.com/watch?v=UYwF-jdcVjY',
        'https://www.youtube.com/watch?v=44VBVj7n91s',
        'https://www.youtube.com/watch?v=SJOgTMP8cs4', 
        'https://www.youtube.com/watch?v=xTlNMmZKwpA', 
        'https://www.youtube.com/watch?v=ir6nk2zrMG0', 
        'https://www.youtube.com/watch?v=y2tEPmwWEiI',
        'https://www.youtube.com/watch?v=5Fv19KVVya8',
        'https://www.youtube.com/watch?v=Bran_v6ZtPI',
        'https://www.youtube.com/watch?v=4IPEcV8_uaM',
        'https://www.youtube.com/watch?v=dTQMd2I3drE',
        'https://www.youtube.com/watch?v=KXanlLjOFh0',
        'https://www.youtube.com/watch?v=Az-mGR-CehY',
        'https://www.youtube.com/watch?v=HuIrr-qJ_UQ',
        'https://www.youtube.com/watch?v=XCQK6LmhYqc',
        'https://www.youtube.com/watch?v=XnyHYeCsXJ8'
    ], 
    name: [
        'Post Malone - Better Now',
        'Alan Walker Ft. Sofia Carson, K-391 & CORSAK - Different World',
        'The Chainsmokers - Hope ft. Winona Oak',
        'Cardi B, Bad Bunny & J Balvin - I Like It',
        'Calvin Harris, Rag\'n\'Bone Man - Giant',
        'DHARIA - Sugar & Brownies',
        'KAZKA - Plakala',
        'Clean Bandit Ft. Ellie Goulding - Mama',
        'Havana Ft. Yaar - I Lost You',
        'Calvin Harris Ft. Sam Smith - Promises',
        'Jax Jones, Mabel Ft. Rich The Kid - Ring Ring',
        'K-391 & Alan Walker Ft. Julie Bergan & Seungri - Ignite',
        'Jax Jones, Years & Years - Play',
        'Rita Ora - Let You Love Me',
        'Clean Bandit Ft. Charli XCX & Bhad Bhabie - Playboy Style'
    ]
}

module.exports = ytList